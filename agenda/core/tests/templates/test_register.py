...
class Register_OK_Test(TestCase):
    ...
    def test_register_post_data(self):
        ...
        data= {
            ...
            'telefone' : '19999999887',
            'email' : 'test.test2@fatec.sp.gov.br',
            'observacao' : 'teste'
        }
        response = self.client.post(self.register_url, data)
        self.assertEqual(response.status_code, HTTPStatus.FOUND)
        self.assertRedirects(response, reverse('home'))
        self.assertTrue(Agenda.objects.filter(nome_completo='rafaela morais').exists())
    ...
    def test_register_invalid_phone(self):
        ...
        data= {
            'nome_completo' : 'rafaela morais',
            'telefone' : '16655659987898742',
            'email' : 'test.teste2@fatec.sp.gov.br',
            'observacao' : 'teste'
        }
        response = self.client.post(self.register_url, data)
        self.assertEqual(response.status_code, HTTPStatus.OK)
        ...
        self.assertFalse(Agenda.objects.filter(nome_completo='rafaela morais').exists())

    def test_name_with_special_characters(self):
        self.client.login(email='admin@fatec.sp.gov.br', password='fatec', username='admin')    
        data= {
            'nome_completo' : 'rafaela morais 1 @#$%',
            'telefone' : '19998755412',
        ...
        self.assertFalse(Agenda.objects.filter(nome_completo='rafaela morais 1 @#$%').exists())
